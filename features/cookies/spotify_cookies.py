...
from main.config import logger, SPOTIFY_COOKIE_VIDEO_ID, COOKIES_PATH
...
async def set_spotify_cookies(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    awaiting_cookie[user_id] = "spotify"

    await query.message.reply_text(
        "📥 To enable Spotify downloads, please follow these steps to provide your cookies data securely:\n\n"
        ...
        "2. Open <a href=\"https://accounts.spotify.com/en/login\">spotify.com</a> and <b>log in</b> using your Spotify credentials.\n\n"
    ...
    await query.message.reply_video(
        video=SPOTIFY_COOKIE_VIDEO_ID,
        caption="Here’s a quick video guide showing how to export and upload your Spotify cookies."
    )
    await query.message.reply_text(
...
################################
# 🔘 DISCONNECT SPOTIFY CALLBACK
################################
async def remove_spotify_cookies(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    ...
    file_path = COOKIES_PATH / "spotify" / f"{vareon_id}.txt"

    if os.path.exists(file_path):
        os.remove(file_path)

    from features.cookies.set_cookies import cookies
